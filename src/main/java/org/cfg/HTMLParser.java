...
//		}
	...
	private static void processKVRow(Element label, Element value, Map<String, Object> jsonContainer) {
		...
		List<String> tokens = Lists.newArrayList(Splitter.on("\u0001").omitEmptyStrings().trimResults().split(value.text()));
		if (tokens.size()==1) {		
...
//			String lowerKey = key.toLowerCase();
//			System.out.println(lowerKey);
//			String newKey = lowerKey.substring(0, lowerKey.indexOf(" date(s)") + 5).replaceAll("[()]", "").replaceAll(" ", "_");
//			System.out.println(newKey);
//			List<Date> d = extractDates(value.text());
//			if (d != null && !d.isEmpty()) {
...
//		}
	...
	private static void processCDFAs(Element e, Map<String, Object> jsonContainer) {
		List<String> tokens = Lists.newArrayList(Splitter.on(";").omitEmptyStrings().trimResults().split(e.text()));
